...
class BaiKe:
    def __init__(self,start_url):
        ...
        self.linkStack = LinkStack()
        self.linkStack.addUnvisitedUrl(start_url)
    ...
    def crawl(self,carwl_deepth):
        while(self.current_deepth < carwl_deepth):
            ...
            if not self.linkStack.UnvisitedUrlsEmpty():
                url_to_visit = self.linkStack.UnvisitedUrlStackPop()
                ...
                newLinks = self.getHyperLinks(html)
                self.linkStack.addVisitedUrl(url_to_visit)
                for i in newLinks:
                    self.linkStack.addUnvisitedUrl(i)
                self.current_deepth += 1
            
        else:
            while not self.linkStack.UnvisitedUrlsEmpty():
                url_to_visit = self.linkStack.UnvisitedUrlDeQueue()
                key = parse.unquote(url_to_visit[url_to_visit.find('item/')+5:])
                lemmaid = url_to_visit[url_to_visit.rfind('/')+1:]
                html =  self.getHtml(url_to_visit)
                summary = self.parseItem(html)
                self.Pipeline(key,summary,lemmaid)
